...
public class OllivandersSchedule implements Runnable {
	...
	public void run() {
		...
		// Reset the timer so it does not grow unbounded, use >= just in case a tick
		// gets missed somehow
	...
	/**
	 * Scheduling method that calls checkEffect() on all SpellProjectile objects and
	 * removes those that have kill set to true.
	 */
	...
	/**
	 * Scheduling method that calls checkEffect on all OEffect objects associated
	 * with every online player and removes those that have kill set to true.
	 */
	...
	/**
	 * Hides a player with the Cloak of Invisibility from other players. Also hides
	 * players in Repello Muggleton from players not in that same spell. Also sets
	 * any Creature targeting this player to have null target.
	 */
		...
		for (Player player : p.getServer().getOnlinePlayers()) {
			...
			if (hasCloak || inRepelloMuggletons) {
				for (Player player2 : p.getServer().getOnlinePlayers()) {
					if (player2.isPermissionSet("Ollivanders2.BYPASS")
							&& player2.hasPermission("Ollivanders2.BYPASS")) {
						continue;
					}
		...
		for (Ollivanders2TeleportEvents.O2TeleportEvent event : teleportEvents) {
			...
			try {
				...
				if (curWorld == null || destWorld == null) {
					Ollivanders2API.common.printDebugMessage("OllvandersSchedule.teleportSched: world is null", null,
							null, true);
				} else {
