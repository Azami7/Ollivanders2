...
/**
 * Nullum apparebit creates a stationary spell which will not allow apparition
 * into it.
 * <p>
...
public class NULLUM_APPAREBIT extends O2StationarySpell {
	 ...
	 */
	public NULLUM_APPAREBIT(@NotNull Ollivanders2 plugin, @NotNull UUID pid, @NotNull Location location, int radius,
			int duration) {
		super(plugin);
		spellType = O2StationarySpellType.NULLUM_APPAREBIT;
	...
	void doOnOllivandersApparateByCoordinatesEvent(@NotNull OllivandersApparateByCoordinatesEvent event) {
		...
		if (isLocationInside(destination)) {
			event.setCancelled(true);
			common.printDebugMessage("NULLUM_APPAREBIT: canceled OllivandersApparateByCoordinatesEvent", null, null,
					false);
		}
