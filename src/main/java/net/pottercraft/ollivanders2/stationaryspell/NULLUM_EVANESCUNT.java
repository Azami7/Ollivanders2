...
/**
 * Nullum evanescunt creates a stationary spell which will not allow apparition
 * out of it.
 * <p>
...
public class NULLUM_EVANESCUNT extends O2StationarySpell {
	 ...
	 */
	public NULLUM_EVANESCUNT(@NotNull Ollivanders2 plugin, @NotNull UUID pid, @NotNull Location location, int radius,
			int duration) {
		super(plugin);
		spellType = O2StationarySpellType.NULLUM_EVANESCUNT;
	...
	void doOnOllivandersApparateByCoordinatesEvent(@NotNull OllivandersApparateByCoordinatesEvent event) {
		...
		if (isLocationInside(playerLocation)) {
			event.setCancelled(true);
			common.printDebugMessage("NULLUM_EVANESCUNT: canceled OllivandersApparateByCoordinatesEvent", null, null,
					false);
		}
