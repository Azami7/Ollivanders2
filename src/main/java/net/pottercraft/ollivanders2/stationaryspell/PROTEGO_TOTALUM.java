...
public class PROTEGO_TOTALUM extends ShieldSpell {
	 ...
	 */
	public PROTEGO_TOTALUM(@NotNull Ollivanders2 plugin, @NotNull UUID pid, @NotNull Location location, int radius,
			int duration) {
		super(plugin);
		spellType = O2StationarySpellType.PROTEGO_TOTALUM;
	...
	void doOnPlayerMoveEvent(@NotNull PlayerMoveEvent event) {
		...
		if (toLoc == null || toLoc.getWorld() == null || fromLoc.getWorld() == null
				|| Ollivanders2Common.isInside(fromLoc, location, radius))
			return;
			...
			new BukkitRunnable() {
				...
				public void run() {
					if (event.isCancelled()) {
						flair(10);
						event.getPlayer()
								.sendMessage(Ollivanders2.chatColor + "A magical force prevents you moving here.");
					}
				}
