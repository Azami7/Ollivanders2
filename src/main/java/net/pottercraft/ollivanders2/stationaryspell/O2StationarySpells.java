...
public class O2StationarySpells implements Listener {
	...
	public void addStationarySpell(@NotNull O2StationarySpell spell) {
		common.printDebugMessage("O2StationarySpells.addStationarySpell: adding " + spell.getSpellType().toString()
				+ " with duration " + spell.duration + " and radius of " + spell.radius, null, null, false);
		O2StationarySpells.add(spell);
	}

	/**
	 * Remove a stationary spell. Since we do not want to lose track of a spell, we
	 * set kill to true and let upkeep() clean it up.
	 *
	...
	public void removeStationarySpell(@NotNull O2StationarySpell spell) {
		common.printDebugMessage(
				"O2StationarySpells.removeStationarySpell: removing " + spell.getSpellType().toString(), null, null,
				false);
		spell.kill();
	}
	 ...
	 */
	@NotNull
	public List<O2StationarySpell> getActiveStationarySpellsAtLocationByType(@NotNull Location location,
			@NotNull O2StationarySpellType spellType) {
	 ...
	 */
	public boolean checkLocationForSpell(@NotNull Location location,
			@NotNull O2StationarySpellType stationarySpellType) {
	...
	public void upkeep() {
		...
		for (O2StationarySpell statSpell : s) {
			...
			if (statSpell.kill) {
				common.printDebugMessage("O2StationarySpells.upkeep: removing " + statSpell.getSpellType().toString(),
						null, null, false);
	...
	private List<O2StationarySpell> deserializeO2StationarySpells(@NotNull List<Map<String, String>> serializedSpells) {
		...
		for (Map<String, String> spellData : serializedSpells) {
			...
			// caster uuid
			//
			if ((statSpell == null) || !spellData.containsKey(playerUUIDLabel))
				continue;
	...
	/**
	 * Create a basic stationary spell object by type. This will not have all the
	 * fields set to be active so will be set inactive by default.
	 *
	...
	public O2StationarySpell getStationarySpellByType(@NotNull O2StationarySpellType spellType) {
		...
		try {
			...
			common.printDebugMessage("Exception trying to create new instance of " + spellType.toString(), e, null,
					true);
			return null;
		}
