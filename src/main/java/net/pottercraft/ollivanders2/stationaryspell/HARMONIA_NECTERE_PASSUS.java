...
public class HARMONIA_NECTERE_PASSUS extends O2StationarySpell {
	 ...
	 */
	public HARMONIA_NECTERE_PASSUS(@NotNull Ollivanders2 plugin, @NotNull UUID pid, @NotNull Location location,
			@NotNull Location twin) {
		super(plugin);
	...
	public HARMONIA_NECTERE_PASSUS getTwin() {
		for (O2StationarySpell stationarySpell : Ollivanders2API.getStationarySpells().getActiveStationarySpells()) {
			if (stationarySpell instanceof HARMONIA_NECTERE_PASSUS
					&& Ollivanders2Common.locationEquals(stationarySpell.location, twinCabinetLocation))
	 ...
	 *
	 * @param feet the block at the player's feet if the player is standing in the
	 *             cabinet
	...
	private boolean cabinetCheck(@NotNull Block feet) {
		...
		return (feet.getRelative(1, 0, 0).getType() != Material.AIR
				...
				&& feet.getRelative(0, 0, -1).getType() != Material.AIR
				&& feet.getRelative(0, 1, -1).getType() != Material.AIR
				&& feet.getRelative(0, 1, 0).getType() == Material.AIR
	...
	void doOnPlayerMoveEvent(@NotNull PlayerMoveEvent event) {
		...
		// make sure they do not have a use cooldown on the twin or we'll just teleport
		// them right back as soon as they arrive and trigger a move event
