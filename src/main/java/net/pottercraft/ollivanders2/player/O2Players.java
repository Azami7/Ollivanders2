...
public class O2Players {
	...
	/**
	 * A count of the player records read at start. This can be used to prevent
	 * writing back out at server shut down if something goes wrong on plugin load.
	 */
	...
	/**
	 * Get an O2Player by name. This is inefficient compared to by UUID and should
	 * only be used when UUID is not available.
	 *
	...
	/**
	 * Serialize the O2Player to a list of key value pairs for all the data we want
	 * to save.
	 * <p>
	 * Map structure:<br>
	 * Key - UUID<br>
	 ...
	 * </pre>
	 *
	 * @param o2PlayerMap a map of all player MC UUIDs and corresponding O2Player
	 *                    object
	...
	private Map<String, Map<String, String>> serializeO2Players(@NotNull Map<UUID, O2Player> o2PlayerMap) {
		...
		if (recordCount != 0) {
			...
			if (o2PlayerMap.size() < (recordCount / 2))
				p.getLogger().info(
						"Player list is less than half the size when the server started, this may indicate a problem.");
		}
	 ...
	 * @return the deserialized map of O2Players, null if map could not be
	 *         deserialized
	 */
	@Nullable
	...
	/**
	 * Correct a player's animagus color form - for use when valueOf() on the
	 * current value fails
	 *
	...
	public boolean runYear(@NotNull CommandSender sender, @NotNull String[] args) {
		...
		if (!Ollivanders2.useYears) {
			sender.sendMessage(Ollivanders2.chatColor + "Years are not currently enabled for your server."
		...
		if (args.length == 2) {
			...
			if (o2p != null)
				sender.sendMessage(Ollivanders2.chatColor + "Player " + playerName + " is a "
						+ o2p.getYear().getDisplayText() + " year.");
			else
	 ...
	 * @return true unless an error occurred
	 */
	private boolean runYearSet(@NotNull CommandSender sender, @NotNull String targetPlayer,
			@NotNull String targetYear) {
		if (!sender.hasPermission("Ollivanders2.admin"))
			return false;
	...
	private void usageYear(@NotNull CommandSender sender) {
		sender.sendMessage(Ollivanders2.chatColor + "Year commands: "
	...
	public void playerSummary(@NotNull CommandSender sender, @NotNull Player player) {
		...
		// wand type
		if (o2p.foundWand()) {
			String wandlore = Ollivanders2API.getItems().getWands().createLore(o2p.getDestinedWandWood(),
					o2p.getDestinedWandCore());
			summary.append("\nWand Type: ").append(wandlore);

			O2SpellType masterSpell = o2p.getMasterSpell();
			if (masterSpell != null)
				summary.append("\nMaster Spell: ")
						.append(Ollivanders2Common.enumRecode(masterSpell.toString().toLowerCase()));
		...
		if (knownSpells.size() > 0) {
			...
			for (O2SpellType spellType : O2SpellType.values()) {
				if (knownSpells.containsKey(spellType) && Ollivanders2API.getSpells().isLoaded(spellType))
					summary.append("\n* ").append(spellType.getSpellName()).append(" ")
							.append(knownSpells.get(spellType).toString());
			}
		} else {
		...
		if (!knownPotions.isEmpty()) {
			...
			for (O2PotionType potionType : O2PotionType.values()) {
				if (knownPotions.containsKey(potionType) && Ollivanders2API.getPotions().isLoaded(potionType))
					summary.append("\n* ").append(potionType.getPotionName()).append(" ")
							.append(knownPotions.get(potionType).toString());
			}
		} else {
			if (sender.getName().equalsIgnoreCase(player.getName()))
