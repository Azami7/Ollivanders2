 ...
 * All magic books.
 * <p>
 * When bookLearning is enabled, reading a book will increase the reader's spell
 * level by 1 to a maximum of 10. This can be used for classes, creating
 * lessons, or other in-game magic learning.
 * <p>
 * When bookLEarning is enabled, every Ollivanders2 spell must be in a
 * WrittenBook with lore or NBT set up correctly or players will not be able to
 * learn them.
 * <p>
 * Reference:
...
public final class O2Books implements Listener {
	 ...
	 * <p>
	 * This needs to be run after spells and potions are loaded or the text for
	...
	private void readBook(@NotNull Player player, @NotNull ItemStack book) {
		common.printDebugMessage(player.getDisplayName() + " reading a book and book learning is enabled.", null, null,
				false);

		// reading a book, if it is a spell book we want to let the player "learn" the
		// spell.
	...
	private static void incrementSpell(@NotNull O2Player o2p, @NotNull O2SpellType spellType, @NotNull Ollivanders2 p) {
		...
		if (spellLevel < 25) {
			...
			if (Ollivanders2API.getPlayers().playerEffects.hasEffect(o2p.getID(),
					O2EffectType.IMPROVED_BOOK_LEARNING)) {
				o2p.incrementSpellCount(spellType);
			}
		}
	}
	...
	/**
	 * If bookLearning is enabled, read the lore for a book and learn all spells not
	 * yet known.
	 *
			...
			if (sender instanceof Player) {
				...
				if (bookItem == null) {
					sender.sendMessage(Ollivanders2.chatColor
							+ "Unable to find that book. Are you sure you spelled the title correctly?");
					return true;
				}
	...
	public static boolean matchesO2Book(@NotNull ItemStack book, @NotNull O2BookType bookType,
		...
		if ((title == null) || !(title.equalsIgnoreCase(bookType.getShortTitle(p))))
			return false;
