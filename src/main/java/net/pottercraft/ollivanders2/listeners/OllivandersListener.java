...
public class OllivandersListener implements Listener {
	 ...
	 */
	private void updateRecipients(@NotNull Player player, @NotNull O2SpellType spellType,
		...
		for (Player recipient : temp) {
			...
			if (!Ollivanders2Common.isInside(location, recipient.getLocation(), Ollivanders2.chatDropoff)) {
				try {
					...
					common.printDebugMessage("OllivandersListener.updateRecipient: exception removing recipient", e,
							null, true);
				}
			}
		}
	...
	private void doSpellCasting(@NotNull Player player, @NotNull O2SpellType spellType, @NotNull String[] words) {
		if (p.canCast(player, spellType, true)) {
			if (Ollivanders2.bookLearning && p.getO2Player(player).getSpellCount(spellType) < 1) {
				// if bookLearning is set to true then spell count must be > 0 to cast this
				// spell
				common.printDebugMessage("doSpellCasting: bookLearning enforced", null, null, false);
				player.sendMessage(Ollivanders2.chatColor
						+ "You do not know that spell yet. To learn a spell, you'll need to read a book about that spell.");
			...
			common.printDebugMessage("doSpellCasting: Either no spell cast attempted or not allowed to cast", null,
					null, false);
		}
	}
	...
	private O2Spell createSpellProjectile(@NotNull Player player, @NotNull O2SpellType name, double wandC) {
		...
		try {
			...
			common.printDebugMessage("OllivandersListener.createSpellProjectile: exception creating spell constructor",
					e, null, true);
			return null;
		}
		...
		try {
			...
			common.printDebugMessage("OllivandersListener.createSpellProjectile: exception creating spell", e, null,
					true);
			return null;
		}
	...
	private void castSpell(@NotNull Player player) {
		...
		if (spellType != null) {
			...
			if (playerHoldsWand) {
				common.printDebugMessage("OllivandersListener:castSpell: player holds a wand in their primary hand",
						null, null, false);
				wandCheck = Ollivanders2API.playerCommon.wandCheck(player, EquipmentSlot.HAND);
			} else {
				common.printDebugMessage(
						"OllivandersListener:castSpell: player does not hold a wand in their primary hand", null, null,
						false);
				return;
			}
			...
			common.printDebugMessage(
					"OllivandersListener:castSpell: " + player.getName() + " cast " + castSpell.getName(), null, null,
					false);
	...
	private void primaryHandInteractEvents(@NotNull PlayerInteractEvent event) {
		...
		//
		// A right or left click of the primary hand when holding a wand is used to make
		// a magical action.
		//
		if ((Ollivanders2API.getItems().getWands().holdsWand(player, EquipmentSlot.HAND))) {
			...
			// - to brew a potion if they are holding a glass bottle in their off hand and
			// facing a cauldron
			//
				...
				if ((cauldron != null)
						&& (player.getInventory().getItemInOffHand().getType() == Material.GLASS_BOTTLE)) {
				...
				// play a sound and visual effect when they right-click their destined wand with
				// no spell
	...
	/**
	 * If non-verbal spell casting is enabled, selects a new spell from mastered
	 * spells.
	 *
	...
	private void rotateNonVerbalSpell(@NotNull Player player, @NotNull Action action) {
		...
		if (spell != null) {
			String spellName = Ollivanders2Common
					.firstLetterCapitalize(Ollivanders2Common.enumRecode(spell.toString()));
	...
	/**
	 * This checks if a player kills another player, and if so, adds a soul to the
	 * attacking o2player
	 *
	...
	/**
	 * When an item is picked up by a player, if the item is a portkey, the player
			...
			for (String s : temp) {
				if (s.startsWith(PORTUS.portus)) {
					...
					to = new Location(Bukkit.getServer().getWorld(UUID.fromString(portArray[1])),
							Double.parseDouble(portArray[2]), Double.parseDouble(portArray[3]),
	...
	/**
	 * When a player consumes something, see if it was a potion and apply the effect
	 * if it was.
	 *
			...
			if (container.has(O2Potions.potionTypeKey, PersistentDataType.STRING) || meta.hasLore()) {
				...
				if (potion != null) {
					if (!Ollivanders2.libsDisguisesEnabled
							&& Ollivanders2Common.libDisguisesPotions.contains(potion.getPotionType()))
						return;
	...
	/**
	 * When a user holds their spell journal, replace it with an updated version of
	 * the book.
	 * <p>
	 * Reference:
	 ...
	 * @apiNote assumes player is holding a glass bottle in their off-hand and will
	 *          set off-hand item to null
	 */
	private void brewPotion(@NotNull Player player, @NotNull Block cauldron) {
		...
		if (Ollivanders2Common.hotBlocks.contains(under.getType())) {
			...
			if (potion == null) {
				player.sendMessage(Ollivanders2.chatColor
						+ "The cauldron appears unchanged. Perhaps you should check your recipe");
				return;
			}
	...
	private boolean divine(@NotNull O2SpellType spellType, @NotNull Player sender, @NotNull String[] words) {
		...
		if (words.length < 2) {
			sender.sendMessage(
					Ollivanders2.chatColor + "You must say the name of the player. Example: 'astrologia steve'.");
			return false;
		}
