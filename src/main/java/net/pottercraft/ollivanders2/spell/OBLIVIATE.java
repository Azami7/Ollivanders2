...
/**
 * Decreases all of target player's spell levels by the caster's level in
 * obliviate.
 * <p>
...
public final class OBLIVIATE extends O2Spell {
	...
	/**
	 * Target the first player we find and remove random potion and/or spell
	 * knowledge
	 */
	@Override
	...
	/**
	 * Make a player lose a random amount of skill in a potion or spell randomly
	 * selected.
	 *
	...
	private void forgetSomething(O2Player o2player) {
		...
		if (Ollivanders2Common.random.nextInt() > 0 && !knownPotions.isEmpty()) // forget a potion if any known and rand
																				// is positive (50% chance)
		{
			...
			common.printDebugMessage(
					o2player.getPlayerName() + " loses " + reduction + " level in " + potions[index].getPotionName(),
					null, null, false);
		} else // forget a spell
		{
			...
			common.printDebugMessage(
					o2player.getPlayerName() + " loses " + reduction + " level in " + spells[index].getSpellName(),
					null, null, false);
		}
	}
}