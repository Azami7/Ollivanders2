...
public abstract class ItemEnchant extends O2Spell {
	...
	/**
	 * The list of item types that this can enchant, if limited. When empty, all
	 * item types can be enchanted
	 */
	...
	/**
	 * Magnitude of this enchantment - this is the final value used to determine the
	 * enchantment effect
	 */
	int magnitude;
	...
	protected void doCheckEffect() {
		...
		for (Item item : items) {
			...
			if (Ollivanders2API.getItems().getWands().isWand(item.getItemStack())
					|| (Ollivanders2API.getItems().enchantedItems.isEnchanted(item)))
				continue;
	...
	private Item enchantItem(@NotNull Item item) {
		...
		// update original itemStack to remove 1 item, if it had more than one, or
		// remove the original item if there is only 1
