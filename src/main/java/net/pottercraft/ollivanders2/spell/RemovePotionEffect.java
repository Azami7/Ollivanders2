...
public abstract class RemovePotionEffect extends O2Spell {
	...
	void removePotionEffects(@NotNull LivingEntity target) {
		for (PotionEffectType effectType : potionEffectTypes) {
			if (checkSuccess()) {
				common.printDebugMessage("Removing " + effectType.getName() + " from " + target.getName(), null, null,
						false);
	...
	/**
	 * Determine if this spell is successful based on player skill and level of the
	 * effect relative to the level of this spell.
	 *
