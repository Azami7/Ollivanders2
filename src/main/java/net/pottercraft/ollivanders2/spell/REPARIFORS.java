...
/**
 * Reparifors is a healing spell that reverts minor magically-induced ailments,
 * such as paralysis and poisoning.
 * <p>
...
public class REPARIFORS extends O2Spell {
	...
	protected void doCheckEffect() {
		...
		for (Player target : getNearbyPlayers(defaultRadius)) {
			...
			if (Ollivanders2API.getPlayers().playerEffects.hasEffect(target.getUniqueId(), O2EffectType.IMMOBILIZE)
					&& !(Ollivanders2API.getPlayers().playerEffects.hasEffect(target.getUniqueId(),
							O2EffectType.SUSPENSION))) {
				int decrease = ((int) usesModifier / 20) + 5; // age by at least 5 percent

				Ollivanders2API.getPlayers().playerEffects.ageEffectByPercent(target.getUniqueId(),
						O2EffectType.IMMOBILIZE, decrease);
			...
			// do a minor heal
			int duration = (((int) usesModifier / 10) * Ollivanders2Common.ticksPerSecond)
					+ (15 * Ollivanders2Common.ticksPerSecond);
