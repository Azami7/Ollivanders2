...
public final class MORTUOS_SUSCITATE extends ItemToEntityTransfiguration {
	...
	/**
	 * The maximum number of inferi any player can create so this doesn't get out of
	 * control
	 */
	...
	/**
	 * Determine whether this player can create more inferi based on their skill
	 * level
	 */
	@Override
	void doInitSpell() {
		...
		if (inferiCount.containsKey(player)) {
			...
			if (curInferi >= playerMax) {
				// if the number they currently have created exceeds their max based on skill,
				// set success to 0
				successRate = 0;
			}
		}
