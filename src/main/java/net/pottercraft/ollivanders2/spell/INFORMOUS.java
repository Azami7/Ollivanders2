 ...
 * and weather (duration) and Player (spell effects).
 * <p>
 * Reference: https://harrypotter.fandom.com/wiki/Informous_Spell
 */
public final class INFORMOUS extends O2Spell {
	...
	/**
	 * Give information about an entity, stationary spells at the target, or the
	...
	protected void doCheckEffect() {
		...
		if (!gaveInfo) {
			...
			if (playerLocation.getY() > 256) {
				...
				player.sendMessage(Ollivanders2.chatColor + "There will be " + weather + " for "
						+ weatherTime / Ollivanders2Common.ticksPerSecond + " more seconds.");
				if (thunder) {
					player.sendMessage(Ollivanders2.chatColor + "There will be thunder for "
							+ thunderTime / Ollivanders2Common.ticksPerSecond + " more seconds.");
				}
			}
		}
	...
	private void entityInfo(@NotNull LivingEntity entity) {
		...
		if (entity instanceof Player) {
			...
			// detectable effects
			String infoText = Ollivanders2API.getPlayers().playerEffects
					.detectEffectWithInformous(entity.getUniqueId());
			if (infoText != null)
	...
	private void stationarySpellInfo(@NotNull O2StationarySpell spell) {
		if (spell instanceof COLLOPORTUS) {
			...
			player.sendMessage(Ollivanders2.chatColor + spell.getSpellType().toString() + " of radius "
					+ spell.getRadius() + " has " + power + " power left.");
			...
			player.sendMessage(Ollivanders2.chatColor + spell.getSpellType().toString() + casterString + " of radius "
					+ spell.getRadius());
			...
			player.sendMessage(
					Ollivanders2.chatColor + spell.getSpellType().toString() + " of radius " + spell.getLocation()
							+ " has " + spell.getDuration() / Ollivanders2Common.ticksPerSecond + " seconds left.");
		}
	}
}