...
/**
 * Sets fire to blocks or living entities for an amount of time depending on the
 * player's spell level.
 */
public abstract class IncendioSuper extends O2Spell {
	...
	/**
	 * Set entities or the target block on fire or, if already burning, countdown
	...
	protected void doCheckEffect() {
		...
		if (burning) {
			...
			for (Item item : items) {
				item.setFireTicks(timeRemaining);
