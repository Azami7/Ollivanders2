...
public final class CRESCERE_PROTEGAT extends O2Spell {
	...
	/**
	 * Look for a stationary spell at the projectile's location and increase its
	 * radius.
	 * <p>
	 * 1. Target spell must have been cast by this caster. 2. Target spell cannot be
	 * higher difficulty level than this spell. 3. Target spell cannot be on the
	 * spell blocked list (ex. floo network, vanishing cabinets)
	 * <p>
	...
	protected void doCheckEffect() {
		...
		for (O2StationarySpell spell : Ollivanders2API.getStationarySpells().getStationarySpellsAtLocation(location)) {
			...
			if (!spellBlockedList.contains(spell.getSpellType()) && spell.getCasterID().equals(player.getUniqueId())
