...
public class O2Spells {
	...
	/**
	 * Globally allowed spells. If this is set, it will take precedence over
	 * disallowed spells.
	 */
	private ArrayList<O2SpellType> globalDisallowedSpells = new ArrayList<>();

	/**
	 * Globally disallowed spells. If this is set, it will take precedence over
	 * regionally disallowed spells.
	 */
	...
	private O2Spell getSpellFromType(@NotNull O2SpellType spellType) {
		...
		try {
			...
			common.printDebugMessage("Exception trying to create new instance of " + spellType.toString(), exception,
					null, true);
			return null;
		}
	...
	/**
	 * Verify this spell type is loaded. A spell may not be loaded if it depends on
	 * something such as LibsDisguises and that dependency plugin does not exist.
	 *
	...
	private void loadZoneConfig(@NotNull String zoneName) {
		...
		try {
			...
			common.printDebugMessage("O2Spells.loadZoneConfig: zone " + zoneName + " has invalid type " + typeString,
					null, null, true);
			return;
		}
		...
		if (type == SpellZone.SpellZoneType.WORLD || type == SpellZone.SpellZoneType.CUBOID) {
			...
			if (world == null || world.length() < 1) {
				common.printDebugMessage("O2Spells.loadZoneConfig: world or cuboid zone " + zoneName
						+ " with no world name set, ignored.", null, null, true);
				return;
			}
		}
		...
		if (type == SpellZone.SpellZoneType.CUBOID) {
			...
			if (areaString == null || areaString.length() < 1) {
				common.printDebugMessage(
						"O2Spells.loadZoneConfig: cuboid zone " + zoneName + " with no area coordinates set, ignored",
						null, null, true);
				return;
			}
			...
			if (area == null) {
				common.printDebugMessage(
						"O2Spells.loadZoneConfig: zone " + zoneName + " has invalid area " + areaString, null, null,
						true);
				return;
			}
		}
	...
	public boolean isSpellTypeAllowed(@NotNull Location location, @NotNull O2SpellType spellType) {
		...
		for (SpellZone zone : spellZones) {
			...
			if (Ollivanders2.worldGuardO2.isLocationInRegionByName(zone.zoneName, location)
					&& zone.allowedSpells.size() > 0)
				return zone.allowedSpells.contains(spellType);
		}
	 ...
	 * @return true if the spell is explicitly disallowed at this location, false
	 *         otherwise
	 */
	private boolean isExplicitlyDisallowed(@NotNull Location location, @NotNull O2SpellType spellType) {
		...
		for (SpellZone zone : spellZones) {
			...
			if (Ollivanders2.worldGuardO2.isLocationInRegionByName(zone.zoneName, location)
					&& zone.disallowedSpells.contains(spellType))
				return true;
		}
