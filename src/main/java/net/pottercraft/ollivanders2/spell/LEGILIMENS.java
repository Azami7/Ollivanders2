...
public final class LEGILIMENS extends O2Spell {
	...
	protected void doCheckEffect() {
		...
		for (Player target : getNearbyPlayers(defaultRadius)) {
			...
			{
				if (Ollivanders2API.getPlayers().playerEffects.hasEffect(target.getUniqueId(),
						O2EffectType.ANIMAGUS_EFFECT)) {
					...
					// when in animagus form, only someone who has mastered legilimens can mind read
					// a person
					if (usesModifier >= 10) {
	...
	private void readMind(Player target) {
		...
		if (rand >= 50) {
			if (o2p.foundWand())
				player.sendMessage(Ollivanders2.chatColor + " uses a " + o2p.getDestinedWandWood() + " and "
						+ o2p.getDestinedWandCore() + " wand.");
			else
		...
		if (usesModifier > 2) {
			...
			if (usesModifier > 3) {
				...
				if (rand >= 66) {
					if (Ollivanders2.enableNonVerbalSpellCasting) {
						...
						if (masteredSpell != null)
							player.sendMessage(Ollivanders2.chatColor + " can non-verbally cast the spell "
									+ masteredSpell.getSpellName() + ".");
					}
				}
				...
				if (usesModifier > 5) {
					...
					if (rand >= 40) {
						String legilText = Ollivanders2API.getPlayers().playerEffects
								.detectEffectWithLegilimens(o2p.getID());
						if (legilText != null)
						...
						if (rand >= 90) {
							if (o2p.isAnimagus()) {
								...
								if (animagusForm != null)
									player.sendMessage(Ollivanders2.chatColor + " has the animagus form of a "
											+ Ollivanders2Common.enumRecode(animagusForm.toString()) + ".");
							}
						}
					}
				}
