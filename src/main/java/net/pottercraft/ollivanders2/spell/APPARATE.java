...
public final class APPARATE extends O2Spell {
	...
	/**
	 * Determine if the distance a player is trying to apparate exceeds the max
	 * apparate distance in the X or Z coordinates.
	 *
	 * @param fromLoc the location they are apparating from
	 * @param toLoc   the location they are apparating to
	 * @return true if the locations are less than maxApparateDistance apart, false
	 *         otherwise
	 */
	private boolean exceedsMaxDistance(@NotNull Location fromLoc, @NotNull Location toLoc) {
		...
		// ensure the player is not trying to go more than maxApparateDistance blocks in
		// the x and z directions
		if ((Math.abs(fromLoc.getX() - toLoc.getX()) > maxApparateDistance)
				|| (Math.abs(fromLoc.getZ() - toLoc.getZ()) > maxApparateDistance))
			return true;
