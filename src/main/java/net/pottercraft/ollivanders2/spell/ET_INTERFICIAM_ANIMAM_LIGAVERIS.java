...
public final class ET_INTERFICIAM_ANIMAM_LIGAVERIS extends O2Spell {
	 ...
	 */
	public ET_INTERFICIAM_ANIMAM_LIGAVERIS(@NotNull Ollivanders2 plugin, @NotNull Player player,
		...
		if (Ollivanders2.worldGuardEnabled)
			worldGuardFlags.add(Flags.MOB_SPAWNING); // needed because Fiendfyre requires it, otherwise horcruxes could
														// get made in locations players couldn't kill them

		initSpell();
	}

	/**
	 * This spell is not a projectile but makes the stationary spell in the player's
	 * current location.
	 */
	@Override
	public void checkEffect() {
		...
		if (healthAttribute == null) {
			common.printDebugMessage("ET_INTERFICIAM_ANIMAM_LIGAVERIS.checkEffect: player health attribute is null",
					null, null, false);
			kill();
			return;
		}
		...
		// If the player's soul isn't s0 split that they can survive making another
		// horcrux, then make a new one and damage them
		if (futureHealth - 1 > 0) {
			...
			// If they player couldn't survive making another horcrux then they are sent
			// back to a previous horcrux
			...
			for (O2StationarySpell stationary : stationaries) {
				if (stationary.getSpellType() == O2StationarySpellType.HORCRUX
						&& stationary.getCasterID().equals(player.getUniqueId())) {
