...
/**
 * The super class for transfiguration of objects. Not for use on players or
 * entities.
 */
public abstract class BlockTransfiguration extends TransfigurationBase {
	...
	/**
	 * A map of the transfigured blocks and their original types for use with
	 * revert()
	 */
	...
	/**
	 * How many blocks out from the target are affects. Usually for permanent spells
	 * this is 1.
	 */
	...
	/**
	 * Transfigure the target block or blocks. Will not change the block if it is on
	 * the materialBlacklist list or if the target block is already the
	...
	protected void transfigure() {
		...
		// get the objects to be transfigured, target block is not always the block at
		// the location (such as aguamenti), so we cannot use 'location'
	...
	boolean canTransfigure(@NotNull Block block) {
		common.printDebugMessage("BlockTransfigure.canTranfigure: Checking if this block can be transfigured.", null,
				null, false);
		...
		if (transfigurationMap.size() > 0 && transfigurationMap.containsKey(transfigureType)
				&& transfigurationMap.get(transfigureType) == blockType) {
			...
			// do not change if this block is already the subject of a temporary
			// transfiguration
	...
	/**
	 * Spell specific revert actions beyond changing the block type back. Needs to
	...
	public boolean isBlockTransfigured(@NotNull Block block) {
		if (permanent)
			// if this spell is permanent, it is no longer considered as "affecting" the
			// block, the block *is* the new type
			// this condition should never happen because permanent spells should be killed
			// but just in case one is hanging around
			return false;
