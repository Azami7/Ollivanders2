...
/**
 * Shrinks a stationary shield spell's radius. Only the player who created the
 * stationarySpellObject can change it's size.
 * <p>
 * Shield spell distinction is to prevent this changing things like floo network
 * and vanishing cabinets.
 */
public final class HORREAT_PROTEGAT extends O2Spell {
	...
	/**
	 * Reduce the radius of any stationary shield spells within a radius of the
	 * target, if they were cast by this player
	 */
	@Override
	protected void doCheckEffect() {
		...
		for (O2StationarySpell stationarySpell : Ollivanders2API.getStationarySpells().getActiveStationarySpells()) {
			if ((stationarySpell instanceof ShieldSpell) && stationarySpell.isLocationInside(location)
					&& stationarySpell.getCasterID().equals(player.getUniqueId())) {
