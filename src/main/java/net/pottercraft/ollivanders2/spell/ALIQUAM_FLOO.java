...
public final class ALIQUAM_FLOO extends O2Spell {
	...
	/**
	 * Creates an aliquam floo stationary spell at this location if it is a fire
	 * with a sign over it.
	 */
	@Override
	protected void doCheckEffect() {
		...
		if (target != null && (target.getType() == Material.FIRE || target.getType() == Material.CAMPFIRE)) {
			Location statLocation = new Location(location.getWorld(), target.getX() + 0.5, target.getY() + 0.125,
					target.getZ() + 0.5);
			...
			if (Ollivanders2Common.wallSigns.contains(target.getRelative(BlockFace.UP).getType())) {
				...
				String flooName = sign.getLine(0).trim() + " " + sign.getLine(1).trim() + " " + sign.getLine(2).trim()
						+ " " + sign.getLine(3).trim();
					...
					if (stat instanceof net.pottercraft.ollivanders2.stationaryspell.ALIQUAM_FLOO) {
						...
						if (ali.getFlooName().equals(flooName) || ali.getBlock().equals(statLocation.getBlock())) {
							...
							player.sendMessage(Ollivanders2.chatColor
									+ "There is already a fireplace registered with the name " + flooName + ".");
							return;
						}
					}
				}

				net.pottercraft.ollivanders2.stationaryspell.ALIQUAM_FLOO aliquam = new net.pottercraft.ollivanders2.stationaryspell.ALIQUAM_FLOO(
						p, player.getUniqueId(), statLocation, flooName);
				aliquam.flair(20);
