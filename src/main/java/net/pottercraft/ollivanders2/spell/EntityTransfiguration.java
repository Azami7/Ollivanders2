...
public abstract class EntityTransfiguration extends TransfigurationBase {
	...
	/**
	 * A list of Entity types that will not be affected by this spell. Only used if
	 ...
	 * <p>
	 * Will not change the entity if it is on the entity blocked list list or if the
	 * target entity is already the transfiguration type.
	 */
	@Override
	void transfigure() {
		if (hasHitTarget() && !isTransfigured) {
			// we've hit a block and the projectile is stopped but we didn't find anything
			// to transfigure
	 ...
	 */
	void respawnEntity() {
		Entity respawn = transfiguredEntity.getWorld().spawnEntity(transfiguredEntity.getLocation(),
		...
		if (originalEntity instanceof Item && transfiguredEntity instanceof Item) {
			...
			Enchantment enchantment = Ollivanders2API.getItems().enchantedItems
					.getEnchantment(((Item) originalEntity).getItemStack());

			if (enchantment != null) {
				Ollivanders2API.getItems().enchantedItems.addEnchantedItem((Item) transfiguredEntity,
						enchantment.getType(), enchantment.getMagnitude(), enchantment.getArgs());
