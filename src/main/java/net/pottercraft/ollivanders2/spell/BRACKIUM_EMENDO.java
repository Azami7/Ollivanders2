...
/**
 * Healing spell that can be used to mend broken bones. Can also be used to
 * remove bones and can be used defensively against skeletons and withers.
 *
 * @author lownes
 * @author Azami7
 */
public final class BRACKIUM_EMENDO extends O2Spell {
	...
	/**
	 * Find living entity near the projectile - if it is a skeleton, damage it, if
	 * it is a player, heal them.
	 */
	@Override
		...
		if (entities.size() > 0) {
			for (LivingEntity entity : entities) {
				...
				if (type == EntityType.SKELETON || type == EntityType.WITHER_SKULL || type == EntityType.WITHER) {
					...
					common.printDebugMessage("Adding heal potion effect to " + entity.getName() + " for "
							+ durationInTicks + " game ticks.", null, null, false);
