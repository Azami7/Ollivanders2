...
/**
 * Reparifarge will untransfigure the target block or entity, does not work on
...
public final class REPARIFARGE extends O2Spell {
	...
	public void doCheckEffect() {
		if (hasHitTarget()) {
			...
			// check the area around the current projectile location for entities that can
			// be targeted
			for (Entity entity : getCloseEntities(1.5)) {
				if (entity.getUniqueId() == player.getUniqueId())
					continue;

				if (entity instanceof EnderDragonPart) // if this is part of an Ender Dragon, get the parent Dragon
														// entity
	...
	/**
	 * Revert this transfiguration, if it is not permanent and the level of this
	 * transfiguration spell is less than the level of Reparifarge.
	 *
