...
public abstract class O2Spell {
	/**
	 * Max spell lifetime in gameticks, 10 minutes, this is used to ensure a code
	 * bug doesn't create never-ending spells. Permanent and semi-permanent spells
	 * need to use stationary spells or effects.
	 */
	...
	/**
	 * The modifier for this spell based on usage. This is for spells that change
	 * behavior based on the caster's experience.
	 */
	...
	/**
	 * Represents which wand the user was holding. See
	 * Ollivanders2Common.wandCheck()
	 */
	...
	/**
	 * The cooldown for this spell. Spells take mental and physical energy for the
	 * caster and cannot be cast in rapid succession.
	 */
	...
	/**
	 * The description text for this spell in spell books. Required or spell cannot
	...
	/**
	 * A list of block types that this spell is allowed to target. Takes precedence
	 * over deny list.
	 */
	...
	/**
	 * Default constructor should only be used for fake instances of the spell such
	 * as when initializing the book text.
	 */
	 ...
	 * etc. and not on class constants. This must be called by each spell
	 * constructor since usage is based on the specific class type and cannot be
	 * determined in the super-class constructors.
	 */
	void initSpell() {
		if (spellType == null) {
			common.printDebugMessage(
					"O2Spell.initSpell() spell type is null, this probably means initSpell was called in an abstract class.",
					null, null, true);
			kill();
			return;
		}
	...
	/**
	 * The spell-specific initialization based on usage, etc.. Must be overridden by
	 * each spell class that has any initializations.
	 */
	void doInitSpell() {
	}

	/**
	 * Game tick update on this spell - must be overridden in child classes or the
	 * spell exits immediately.
	 */
	 ...
	 */
	public void move() {
		// if this is somehow called when the spell is set to killed or we've already
		// hit a target, do nothing
		...
		p.getServer().getPluginManager()
				.callEvent(new OllivandersSpellProjectileMoveEvent(player, this, prevLoc, location));
		...
		// check the block at this location, if it is not a pass-through block, stop the
		// projectile and check the target
	 ...
	 * <p>
	 * If the block is not on the allow list, is on the blocked list, or the spell
	 * cannot be cast in this location, kills the spell
	 */
	...
	private boolean checkWorldGuard() {
		...
		for (StateFlag flag : worldGuardFlags) {
			if (!Ollivanders2.worldGuardO2.checkWGFlag(player, location, flag)) {
				common.printDebugMessage(
						spellType.toString() + " cannot be cast because of WorldGuard flag " + flag.toString(), null,
						null, false);
				return false;
			}
		}
		...
		for (Entity e : entities) {
			if (e instanceof LivingEntity) {
				if (((LivingEntity) e).getEyeLocation().distance(location) < radius
						|| ((e instanceof EnderDragon || e instanceof Giant)
								&& ((LivingEntity) e).getEyeLocation().distance(location) < (radius + 5))) {
					if (!e.equals(player))
						close.add(e);
					else {
						if (lifeTicks > 1)
	...
	/**
	 * Get all living entities within the radius of projectile, including the caster
	...
	public List<LivingEntity> getNearbyLivingEntities(double radius) {
		...
		// handle also adding the current player when the projectile is at its first
		// position
	...
	/**
	 * Get all players within the radius of projectile, including the caster if
	 * within the radius
	 *
	 * @param radius radius within which to get entities
	...
	/**
	 * Get all damageable entities within the radius of projectile, including the
	 * caster if within the radius
	 *
	...
	/**
	 * Sets the uses modifier that takes into account spell uses, wand type, and
	 * spell level if years are enabled. Returns 10.0 if the uses are 100 and the
	 * right wand is held.
	 */
	protected void setUsesModifier() {
		...
		if (Ollivanders2.maxSpellLevel)
			...
			// if it is not a wandless spell, uses modifier is halved if the player is not
			// using their destined wand,
		...
		if (Ollivanders2.useYears) {
			...
			if (maxLevelForPlayer.ordinal() > (spellType.getLevel().ordinal() + 1))
				...
				usesModifier *= 1.5;
			/*
			 * maxLevelForPlayer.ordinal() == spellType.getLevel().ordinal()) no change to
			 * usesModifier
			 */
	...
	/**
	 * The spell-specific actions taken for each check effect. This must be
	 * overridden by each spell or the spell will do nothing.
	 */
	...
	/**
	 * Reverts any changes made to blocks if the effects are temporary. This must be
	 * overridden by each spell that has a revert action.
	 */
	...
	boolean entityHarmWGCheck(Entity entity) {
		...
		if ((entity instanceof Player && !Ollivanders2.worldGuardO2.checkWGFlag(player, location, Flags.PVP))
				|| (entity instanceof Animals
						&& !Ollivanders2.worldGuardO2.checkWGFlag(player, location, Flags.DAMAGE_ANIMALS)))
			return false;
		...
		if (entity instanceof Vehicle
				&& !Ollivanders2.worldGuardO2.checkWGFlag(player, location, Flags.DESTROY_VEHICLE))
			return false;

		// item frames
		if (entity instanceof ItemFrame
				&& !Ollivanders2.worldGuardO2.checkWGFlag(player, location, Flags.ENTITY_ITEM_FRAME_DESTROY))
			return false;

		// paintings
		if (entity instanceof Painting
				&& !Ollivanders2.worldGuardO2.checkWGFlag(player, location, Flags.ENTITY_PAINTING_DESTROY))
			return false;
