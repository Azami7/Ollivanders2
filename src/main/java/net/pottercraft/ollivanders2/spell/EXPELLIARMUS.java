...
/**
 * Disarms an entity of it's held item, flinging the item in the direction of
 * the caster with force determined by the spell level.
 * <p>
...
public final class EXPELLIARMUS extends O2Spell {
	...
	/**
	 * Look for entities in the projectile location and disarm them if they are
	 * holding something
	 */
	@Override
	protected void doCheckEffect() {
		...
		if (livingEntities.size() > 0) {
			for (LivingEntity entity : livingEntities) {
				...
				item.setVelocity(player.getEyeLocation().toVector().subtract(item.getLocation().toVector()).normalize()
						.multiply(velocity));
