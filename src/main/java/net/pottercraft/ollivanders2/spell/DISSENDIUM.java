...
public final class DISSENDIUM extends O2Spell {
	...
	/**
	 * Move the spell projectile until it hits a block, if that is a trapdoor, open
	 * it and keep it open until the max duration of the spell is reached.
	 */
	@Override
	protected void doCheckEffect() {
		...
		// continue until the spell opens a trapdoor, hits another block type and is
		// killed, or projectile expires
		if (isOpen) {
	...
	private void openTrapDoor() {
		...
		List<O2StationarySpell> spellsAtLocation = Ollivanders2API.getStationarySpells()
				.getStationarySpellsAtLocation(targetLocation);
