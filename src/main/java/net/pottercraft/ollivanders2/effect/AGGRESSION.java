...
/**
 * This effect causes the player to impulsively harm those nearby and causes
 * them to provoke attacks by nearby mobs.
 * <p>
...
public class AGGRESSION extends O2Effect {
	 ...
	 * <p>
	 * Value from 1-10 for how aggressive this player will be with 1 being the
	 * lowest level
	 */
		...
		if (!nearby.isEmpty()) {
			...
			if (Ollivanders2.worldGuardEnabled) {
				if (toDamage instanceof Player
						&& !Ollivanders2.worldGuardO2.checkWGFlag(target, toDamage.getEyeLocation(), Flags.PVP))
					return;
				else if (!(toDamage instanceof Monster) && !Ollivanders2.worldGuardO2.checkWGFlag(target,
						toDamage.getEyeLocation(), Flags.DAMAGE_ANIMALS))
					return;
			}
	...
	/**
	 * Override set permanent so that it cannot be called and alter duration, which
	 * we override to also set the aggression level since this is always permanent.
	 *
