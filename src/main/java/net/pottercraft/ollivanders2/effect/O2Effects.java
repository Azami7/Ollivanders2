...
public class O2Effects implements Listener {
	...
	private class EffectsData {
		...
		synchronized Map<O2EffectType, O2Effect> getPlayerActiveEffects(@NotNull UUID pid) {
			...
			try {
				...
				common.printDebugMessage(
						"O2Effects.getPlayerActiveEffects: failed to acquire mutex in getPlayerActiveEffects", e, null,
						true);
			} finally {
				semaphore.release();
			}
		...
		synchronized O2Effect getActiveEffect(@NotNull UUID pid, @NotNull O2EffectType effectType) {
			...
			try {
				...
				common.printDebugMessage("O2Effects.getActiveEffect: failed to acquire mutex in getActiveEffect", e,
						null, true);
			} finally {
				semaphore.release();
			}
		...
		synchronized Map<O2EffectType, Integer> getPlayerSavedEffects(@NotNull UUID pid) {
			...
			try {
				...
				common.printDebugMessage(
						"O2Effects.getPlayerSavedEffects: failed to acquire mutex in getPlayerSavedEffects", e, null,
						true);
			} finally {
				semaphore.release();
			}
		...
		synchronized void updatePlayerSavedEffects(@NotNull UUID pid, @NotNull Map<O2EffectType, Integer> effects) {
			try {
				...
				common.printDebugMessage(
						"O2Effects.updatePlayerSavedEffects: failed to acquire mutex in updatePlayerSavedEffects", e,
						null, true);
			} finally {
				semaphore.release();
			}
		...
		synchronized void updatePlayerActiveEffects(@NotNull UUID pid, @NotNull Map<O2EffectType, O2Effect> effects) {
			try {
				...
				common.printDebugMessage(
						"O2Effects.updatePlayerActiveEffects: failed to acquire mutex in updatePlayerActiveEffects", e,
						null, true);
			} finally {
				semaphore.release();
			}
		...
		synchronized void resetEffects(@NotNull UUID pid) {
			try {
				...
				common.printDebugMessage("O2Effects.resetEffects: failed to acquire mutex in resetEffects", e, null,
						true);
			} finally {
				semaphore.release();
			}
	...
	/**
	 * Determines if this player is affected by this effect. Only checks active
	 * effects.
	 *
	 * @param pid        the id of the player
	...
	/**
	 * Determines if this player is affected by any effect. Only checks active
	 * effects.
	 *
	 * @param pid the id of the player
	 * @return true if they have this effect, false otherwise
	...
	public synchronized void onJoin(@NotNull UUID pid) {
		...
		for (Entry<O2EffectType, Integer> entry : savedEffects.entrySet()) {
			...
			O2Effect effect;
			try {
				effect = (O2Effect) effectClass.getConstructor(Ollivanders2.class, int.class, UUID.class).newInstance(p,
						duration, pid);
			} catch (Exception e) {
				common.printDebugMessage("O2Effects.onJoin: failed to create class for " + effectType.toString(), e,
						null, true);
	...
	public synchronized void addEffect(@NotNull O2Effect effect) {
		...
		common.printDebugMessage("Added effect " + effect.effectType.toString() + " to " + pid.toString(), null, null,
				false);
	}
	...
	public synchronized void removeEffect(@NotNull UUID pid, @NotNull O2EffectType effectType) {
		...
		effectsData.updatePlayerActiveEffects(pid, playerEffects);
		common.printDebugMessage("Removed effect " + effectType.toString() + " from " + pid.toString(), null, null,
				false);
	}
	...
	/**
	 * Run game heartbeat upkeep for all effects for this player. This is not done
	 * for every player because we only want to update the ones that are currently
	 * online, therefore we let the OllivandersScheduler class tell us which ones to
	 * update.
	 *
	 * @param pid the id of the player.
	 */
	...
	public String detectEffectWithInformous(@NotNull UUID pid) {
		common.printDebugMessage("O2Effects.detectEffectWithInformous: detecting effcts with Informous", null, null,
				false);
		...
		common.printDebugMessage(
				"O2Effects.detectEffectWithInformous: found " + activeEffects.keySet().size() + " active effects", null,
				null, false);

		for (O2Effect effect : effects) {
			common.printDebugMessage(
					"O2Effects.detectEffectWithInformous: checking effect " + effect.effectType.toString(), null, null,
					false);
	...
	/**
	 * Get the information for a mind readable effect, if any can be detected. This
	 * is primarily used for the spell Legilimens.
	 * <p>
	...
	public static boolean runCommand(@NotNull CommandSender sender, @NotNull String[] args, @NotNull Ollivanders2 p) {
		if (!(sender instanceof Player) || !sender.hasPermission("Ollivanders2.admin"))
			return false;
	 ...
	 */
	private static void toggleEffect(@NotNull CommandSender sender, @NotNull Player player,
			@NotNull O2EffectType effectType, @NotNull Ollivanders2 p) {
		if (Ollivanders2API.getPlayers().playerEffects.hasEffect(player.getUniqueId(), effectType)) {
			...
			sender.sendMessage(
					Ollivanders2.chatColor + "Removed " + effectType.toString() + " from " + player.getName() + ".\n");
		} else {
			Class<?> effectClass = effectType.getClassName();
			...
			try {
				effect = (O2Effect) effectClass.getConstructor(Ollivanders2.class, int.class, UUID.class).newInstance(p,
						1200, player.getUniqueId());
			} catch (Exception e) {
				sender.sendMessage(Ollivanders2.chatColor + "Failed to add effect " + effectType.toString() + " to "
						+ player.getName() + ".\n");
				e.printStackTrace();
				return;
			}
			...
			if (effect.effectType.isEnabled()) {
				...
				sender.sendMessage(
						Ollivanders2.chatColor + "Added " + effectType.toString() + " to " + player.getName() + ".\n");
			} else
				sender.sendMessage(Ollivanders2.chatColor + effectType.toString()
						+ " is currently disabled in your server config.\n");
		}
	}
