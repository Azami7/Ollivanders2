...
public class ANIMAGUS_EFFECT extends ShapeShiftSuper {
	...
	public ANIMAGUS_EFFECT(@NotNull Ollivanders2 plugin, int duration, @NotNull UUID pid) {
		...
		if (form == null) {
			common.printDebugMessage(
					"Unable to get animagus form for " + Ollivanders2API.getPlayers().getPlayer(pid).getPlayerName(),
					null, null, false);
			kill();
		}
	}
	...
	protected void customizeWatcher() {
		// in case the variant doesn't work, this can be used to fix the value at the
		// end of this method
	...
	/**
	 * Override set permanent to ensure no code can inadvertently make the animagus
	 * effect non-permanent.
	 *
	...
	/**
	 * Cows, cats, and dogs can't fly - no flight-enabled mobs can be transformed in
	 * to
	 *
