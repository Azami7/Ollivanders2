...
/**
 * Class representing a prophecy. Every prophecy involves predicting an effect
 * on a player in the future and has a specific accuracy which changes the
 * likelihood the prophecy will come to pass.
 */
public class O2Prophecy {
	...
	/**
	 * The time until the prophecy will come to pass, in game ticks - one game tick
	 * is 1/20 of a second
	 */
	private long time;
	 ...
	 * @param delayTime      the time, in game ticks, until the prophecy will come
	 *                       to pass, less than 1200 (1 minute) will be rounded up
	 *                       to 1200
	 * @param effectDuration the duration of the effect, 0 for permanent
	 * @param accuracy       the accuracy of this prophecy as a percent from 0 to
	 *                       99, greater than 99 will be rounded down to 99
	 */
	O2Prophecy(@NotNull Ollivanders2 plugin, @NotNull O2EffectType effectType, @NotNull String message,
			@NotNull UUID targetID, @NotNull UUID prophetID, long delayTime, int effectDuration, int accuracy) {
		p = plugin;
	...
	/**
	 * Get the prophecy message, ie. "After the sun sets on the 3rd day, Fred will
	...
	void fulfill() {
		...
		if (accuracy > rand) {
			...
			if (player != null) {
				...
				p.getServer().broadcastMessage(Ollivanders2.chatColor + "And so came to pass the prophecy of "
						+ playerName + ", \"" + prophecyMessage + "\"");
			}
		} else {
			Player prophet = p.getServer().getPlayer(prophetID);
			if (prophet != null)
				prophet.sendMessage(
						Ollivanders2.chatColor + "Your prophecy, \"" + prophecyMessage + "\" did not come to pass.");
		}
