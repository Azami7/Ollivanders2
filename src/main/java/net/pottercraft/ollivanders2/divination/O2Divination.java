...
/**
 * Super class for all divination methods. Creates a prophecy which may come to
 * pass at a future time.
 * <p>
...
public abstract class O2Divination {
	...
	/**
	 * The experience level of the prophet in this type of divination. Affects
	 * accuracy.
	 */
	int experience;
	 ...
	 * Parts of a prophecy:
	 * <p>
	 * 1. prophecyPrefix - optional prefix based on the specific divination method,
	 * such as "Due to the influence of Mars" 2. the time of day - midnight,
	 * sunrise, midday, sunset 3. the day - today, tomorrow, 3rd day, 4th day 4. the
	 * target's name 5. what will happen to them - see A prophecy should read like:
	...
	public void divine() {
		...
		//
		// Calculation:
		// - the prophet gets a 0.5% accuracy per level of experience at this type of
		// divination
		// - the type of divination method has a maximum accuracy level, regardless of
		// skill, which caps accuracy
		//
		...
		O2Prophecy prophecy = new O2Prophecy(p, effectType, finalMessage, targetID, prophetID, ticks, effectDuration,
				accuracy);
	...
	private O2Effect getEffect(@NotNull UUID targetID, @NotNull O2EffectType effectType) {
		...
		try {
			effect = (O2Effect) effectClass.getConstructor(Ollivanders2.class, int.class, UUID.class).newInstance(p, 1,
					targetID);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
