...
/**
 * In newer versions of MC triggering teleport events from AsyncChatEvents is no
 * longer thread-safe. Need to create a queue of owl post events like we use for
...
public class Ollivanders2OwlPost {
	...
	public void processOwlPostRequest(@NotNull Player player, @NotNull String message) {
		...
		if (splitString.length != 3) {
			Ollivanders2API.common.printDebugMessage(
					"Ollivanders2OwlPost.processOwlPostRequest: bad request \"" + message + "\"", null, null, false);
			return;
		}
		...
		if (held.getType() == Material.AIR) {
			player.sendMessage(
					Ollivanders2.chatColor + "No item in your primary hand. Please hold the item you wish to send.");
			return;
		}

		addDelivery(player, recipient.getID(), nearbyEntities.get(0), held);
		Ollivanders2API.common.printDebugMessage(
				"Added owl post delivery from " + player.getName() + " to " + recipient.getPlayerName(), null, null,
				false);

		player.getInventory().setItemInMainHand(null);
		player.sendMessage(
				Ollivanders2.chatColor + "Owl post delivery for " + recipient.getPlayerName() + " scheduled.");
	}
	...
	private boolean doDelivery(Delivery delivery) {
		...
		if (player == null) {
			...
			Ollivanders2API.common.printDebugMessage(
					"Owl post recipient " + delivery.recipient + " offline, deferring delivery", null, null, false);
			return false;
		}

		Location playerLocation = player.getLocation();
		Location deliveryLocation = new Location(playerLocation.getWorld(), playerLocation.getX(),
				playerLocation.getY() + 2, playerLocation.getZ());
		if (deliveryLocation.getWorld() == null) {
			Ollivanders2API.common.printDebugMessage("Ollivanders2OwlPost.doDelivery: delivery location world is null",
					null, null, true);
			return false;
		}
		...
		if (!(blockType == Material.AIR || blockType == Material.CAVE_AIR)) {
			...
			Ollivanders2API.common.printDebugMessage(
					"Owl post recipient " + delivery.recipient + " is in a place the owl cannot go, deferring delivery",
					null, null, false);
			return false;
		}
		...
		new BukkitRunnable() {
			@Override
			public void run() {
				deliverItemToPlayer(delivery.senderName, player, delivery.deliveryPackage, delivery.courier,
						delivery.courierOriginalLocation);
			}
	 ...
	 */
	private void deliverItemToPlayer(String senderName, Player recipient, ItemStack item, Entity courier,
