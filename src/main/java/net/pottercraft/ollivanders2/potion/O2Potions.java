...
public class O2Potions {
	 ...
	 * @return the brewed potion if the recipe matches a known potion, null
	 *         otherwise
	 */
	@Nullable
	public ItemStack brewPotion(@NotNull Block cauldron, @NotNull Player brewer) {
		...
		if (potion == null || (!Ollivanders2.libsDisguisesEnabled
				&& Ollivanders2Common.libDisguisesPotions.contains(potion.getPotionType()))) {
		...
		for (Entity e : cauldron.getWorld().getNearbyEntities(location, 1, 1, 1)) {
			if (e instanceof Item) {
				...
				if (ingredientType == null
						|| material != Ollivanders2API.getItems().getItemMaterialByType(ingredientType))
					continue;

				int count = ((Item) e).getItemStack().getAmount();

				common.printDebugMessage("Found " + count + " of ingredient " + ingredientType.toString(), null, null,
						false);
	...
	public O2Potion findPotionByItemMeta(@NotNull ItemMeta meta) {
		...
		if (meta.hasLore()) {
			...
			// we search on lore rather than item name so that players cannot use anvils to
			// create potions
			if (lore == null)
				return null;
	...
	public O2Potion getPotionFromType(@NotNull O2PotionType potionType) {
		...
		try {
			...
			common.printDebugMessage("Exception trying to create new instance of " + potionType.toString(), exception,
					null, true);
	...
	/**
	 * Verify this potion type is loaded. A potion may not be loaded if it depends
	 * on something such as LibsDisguises and that dependency plugin does not exist.
	 *
