...
public class FLAGRANTE extends Enchantment {
	...
	/**
	 * Flagrate effect the player as soon as the item enters their inventory so we
	 * do not need to check for held
	 *
	 * @param event the item drop event
	 */
	@Override
	...
	/**
	 * A flagrante-cursed item was either held or stopped being held - check to see
	 * if the player is still holding at least 1 flagrante item
	 *
	...
	void checkFlagranteStatus(Player player) {
		if (isHoldingEnchantedItem(player)) {
			...
			Ollivanders2API.getPlayers().playerEffects.removeEffect(player.getUniqueId(),
					O2EffectType.FLAGRANTE_BURNING);
