...
public class VOLATUS extends Enchantment {
	...
	public void doItemDespawn(@NotNull ItemDespawnEvent event) {
		...
		Enchantment enchantment = Ollivanders2API.getItems().enchantedItems
				.getEnchantment(despawnedItem.getItemStack());
		if (enchantment == null || enchantment.getType() != ItemEnchantmentType.VOLATUS)
			return;

		// someone is holding this - we don't know who so we need to iterate through all
		// players to check broom flying
	...
	/**
	 * A broom was either held or stopped being held - check to see if the player is
	 * still holding at least 1 broom
	 *
