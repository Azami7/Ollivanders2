...
public class O2Wands {
	...
	public boolean checkNBT(@NotNull ItemStack itemstack) {
		...
		if (container.has(wandWoodKey, PersistentDataType.STRING)
				&& container.has(wandCoreKey, PersistentDataType.STRING)) {
			// assume if something is set, this is good enough since players cannot set NBT
			// this shouldnt be faked
			return true;
		}
	 ...
	 * @return true if is a wand and it matches the player's destined wand, false
	 *         otherwise
	 */
	...
	public boolean matchesLore(@NotNull String wood, @NotNull String core, @NotNull ItemStack itemStack) {
		...
		if ((wandLore.length != 2) || !wood.equalsIgnoreCase(wandLore[0]) || !core.equalsIgnoreCase(wandLore[1]))
			return false;
	...
	public ItemStack createRandomWand() {
		String wood = O2WandWoodType.getAllWoodsByName()
				.get(Math.abs(Ollivanders2Common.random.nextInt() % O2WandWoodType.getAllWoodsByName().size()));
		String core = O2WandCoreType.getAllCoresByName()
				.get(Math.abs(Ollivanders2Common.random.nextInt() % O2WandCoreType.getAllCoresByName().size()));
