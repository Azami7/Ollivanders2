...
public enum O2ItemType {
	...
	WAND(Material.STICK, (short) 0, "Wand", null, null), WOLFSBANE(Material.ALLIUM, (short) 0, "Wolfsbane", null, null);
	 ...
	 * @param variant     the variant information, this is 0 for most items but for
	 *                    things like potions it can control color
	 * @param name        the name of the item
	 * @param lore        the lore for this item
	 * @param enchantment the optional enchantment for this item
	 */
	O2ItemType(@NotNull Material material, short variant, @NotNull String name, @Nullable String lore,
			@Nullable ItemEnchantmentType enchantment) {
	...
	public boolean isItemThisType(@NotNull ItemStack itemStack) {
		...
		if (container.has(O2Items.o2ItemTypeKey, PersistentDataType.STRING)) {
			...
			if ((itemTypeKey == null) || !(itemTypeKey.equalsIgnoreCase(name)))
				return false;
		}
