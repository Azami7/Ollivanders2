...
public class AddO2Effect extends O2Spell
{
    ...
    */
   public AddO2Effect(Ollivanders2 plugin, Player player, Double rightWand)
   {
    ...
    */
   private void addEffectsToTarget(Player target)
   {
      ...
      for (O2EffectType effectType : effectsToAdd)
      {
         ...
         try
         {
            effect = (O2Effect) effectClass.getConstructor(Ollivanders2.class, Integer.class, UUID.class).newInstance(p, duration, target.getUniqueId());
         }
         catch (Exception e)
         {
